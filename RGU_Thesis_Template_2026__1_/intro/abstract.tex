...
This report covers Parts A and B of COL334 Assignment 1, in which we measure and document the
network infrastructure of the IIT Delhi campus from the vantage point of our own devices, and
then turn the same two devices inward and watch what they say on the wire.
...
Part B reports the traffic-capture half: six ten-minute captures each, over two days and three
time bands. Round-trip time across those twelve windows shows a large, sustained shift on the
off-campus path that follows the calendar date rather than the hour and is visible on two
laptops running different operating systems, while the on-campus evening effect we expected
turns out to be weak and inconsistent between the two devices, a negative result we report as
one. We also account for what each machine sent unasked, and hunt the anomalies planted in our
captures (four in one seed, six in the other), giving each a display filter and the exact
frame count it returns, including the two flavours we can show are genuinely absent rather than
merely missed. Finally we argue against our own result: which anomaly would be hardest to detect
at scale, and what an operator sees that we cannot.
