...
maximum RTT observed for each. Distances are rough straight-line estimates, not independently
verified against ground truth: T2 and T4 from Google Maps, measuring Satpura Hostel to the named
campus/city; T3 and T5 from MaxMind GeoIP resolution of the target IP to its registered
geographic location.
...
\midrule
T1 (local gateway)\footnotemark & 10.184.0.13 & $<$0.1 km & 3.904 & 8.891 & 96.290 \\
...
\end{table}
\footnotetext{Configured gateway is 10.184.0.1; 10.184.0.13 is the first-hop interface that
actually answers ICMP, per Chapter~\ref{ch:Background}, Section~1.2. We ping .13 since it is the
address our traffic demonstrably passes through.}

T4 returned 100\% packet loss across all 50 probes. We read this as ICMP being firewalled at or
near \texttt{www.iitb.ac.in} rather than a routing failure, since traceroutes toward the same
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{report/images/a3_1_rtt_vs_distance_t1t2t3.png}
    \caption[RTT vs.\ approximate distance]{Minimum, median/average, and maximum RTT against approximate distance for T1, T2, and T3. T4 is excluded (100\% packet loss). T5 is excluded from this plot to preserve the linear distance scale, since its nominal 12{,}000\,km would compress T1--T3 into an unreadable cluster near the origin; T5 is analysed in the text below instead.}
    \label{fig:a31plot}
\end{figure}
...
processing delay, not propagation. T5 (\texttt{www.mit.edu}), though excluded from
Figure~\ref{fig:a31plot} to keep that plot's distance axis readable, is the more interesting
case: its 12{,}000\,km distance implies a propagation floor of about 120\,ms round-trip, yet the
measured minimum was 35.6\,ms, well under that floor. The
...
(20-08-2026, 05:40) and again at a busy hour (22-08-2026, 15:11). Figure~\ref{fig:a33plot} plots
RTT against sequence number for both.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{report/images/A3_3_Queue_Saturation.png}
    \caption[Queue saturation, busy vs.\ quiet hour]{RTT vs.\ packet sequence number, 300 pings to T3, quiet hour (20-08-2026, 05:40) vs.\ busy hour (22-08-2026, 15:11). The plot legend rounds the quiet-hour start to 5:30 AM.}
    \label{fig:a33plot}
\end{figure}
