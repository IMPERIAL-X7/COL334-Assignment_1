...
CDN-edge addresses seen only on paths to non-campus destinations -- NKN peering
(\texttt{10.1.200.137}), Meta's peering/transit edge, and various CDN edges for
\texttt{mit.edu}, \texttt{google.com}, and others -- that we left out of Table~\ref{tab:hophunt}
itself to keep it focused on the destinations this report discusses in depth, but which are now
...
at all (as distinct from the hop~5/hop~8 case in Chapter~\ref{ch:Background}, which only
suppressed TTL-exceeded replies while still forwarding transit traffic -- here nothing came back
at all, including to packets that were never expected to traverse these IPs as intermediate
